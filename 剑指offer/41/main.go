...
// 小根堆中存放的是 较大的一般，大根堆存放的是 较小的一半
// 保证小根堆中的数量大于等于大根堆中数量并最多大于1个
...
func (m *MedianFinder) AddNum(num int) {
	m.max.push(num)
	if m.min.Len() != 0 && m.max.top() > m.min.top() {
		a, b := m.max.pop(), m.min.pop()
		m.max.push(b)
		m.min.push(a)
	}
	if m.max.Len() > m.min.Len()+1 {
		m.min.push(m.max.pop())
	}
}

func (m *MedianFinder) FindMedian() float64 {
	if (m.max.Len()+m.min.Len())%2 == 0 {
		return float64(m.max.top()+m.min.top()) / 2.0
	}
	return float64(m.max.top())
}

// 实现 heap 的方法 Pop() interface{} 和 Push(v interface{})
// 并且大根堆需要修改 sort.IntSlice 的比较方法 Less(i,j int) bool
type maxHeap struct{ sort.IntSlice }

func (maxHeap *maxHeap) Less(i, j int) bool { return maxHeap.IntSlice[i] > maxHeap.IntSlice[j] }
func (maxHeap *maxHeap) Push(v interface{}) { maxHeap.IntSlice = append(maxHeap.IntSlice, v.(int)) }
func (maxHeap *maxHeap) Pop() interface{} {
	a := maxHeap.IntSlice
	v := a[len(a)-1]
	maxHeap.IntSlice = a[:len(a)-1]
	return v
}
func (maxHeap *maxHeap) push(v int) { heap.Push(maxHeap, v) }
func (maxHeap *maxHeap) pop() int   { return heap.Pop(maxHeap).(int) }
func (maxHeap *maxHeap) top() int   { return maxHeap.IntSlice[0] }

type minHeap struct{ sort.IntSlice }

func (minHeap *minHeap) Push(v interface{}) { minHeap.IntSlice = append(minHeap.IntSlice, v.(int)) }
func (minHeap *minHeap) Pop() interface{} {
	a := minHeap.IntSlice
	v := a[len(a)-1]
	minHeap.IntSlice = a[:len(a)-1]
	return v
}
func (minHeap *minHeap) push(v int) { heap.Push(minHeap, v) }
func (minHeap *minHeap) pop() int   { return heap.Pop(minHeap).(int) }
func (minHeap *minHeap) top() int   { return minHeap.IntSlice[0] }
...
// // 做完可做 T480. 滑动窗口中位数 https://leetcode.cn/problems/sliding-window-median/
